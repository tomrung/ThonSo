...
wb.SheetNames.forEach(sheetName => {
  ...
  for (let i = 4; i < data.length; i++) {
    const row = data[i];
    if (!row || !row[0] || String(row[0]).toLowerCase().includes('tổng') || String(row[0]).toLowerCase().includes('cộng') || String(row[0]).toLowerCase().includes('đại diện')) continue;
    
    const chuDat = normalizeName(row[1]);
    const hoSanXuat = normalizeName(row[2]) || chuDat;
    let rawXuDong = normalizeName(row[3]);
    
    let xuDong = 'Tổ 9';
    if (rawXuDong.includes('Hà Ra') || sheetName.includes('T10')) {
      xuDong = 'Hà Ra';
    } else if (rawXuDong.includes('La Châu') || sheetName.includes('LaChau')) {
      xuDong = 'La Châu';
    } else if (rawXuDong.includes('LB') || rawXuDong.includes('La Bông')) {
      xuDong = 'La Bông Tây';
    } else if (rawXuDong.includes('Gò') || rawXuDong.includes('ổi') || rawXuDong.includes('Ổi')) {
      xuDong = 'Gò Ổi';
    } else if (rawXuDong.includes('Tổ 9') || sheetName.includes('T9')) {
      xuDong = 'Tổ 9';
    }

    const lo = normalizeName(row[4]) || 'Lô 1';
    const dienTich = Number(row[5]) || 0;
...
const xdCount = {};
records.forEach(r => { xdCount[r.xu_dong] = (xdCount[r.xu_dong] || 0) + 1; });
console.log('REGENERATED SANXUATDATA.TS! Counts:', xdCount, 'Total:', records.length);
